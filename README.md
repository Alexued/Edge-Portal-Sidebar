...
- **全局边缘轨道**：左右侧可选；向内拉出、向外收回，长按后可上下移动，点击外部区域会以非线性动画自动收起。边缘距离可在设置中实时预览和调整；全面屏返回手势下会保留系统安全最小值，避免与返回冲突。
...
- 设置页“行为”中的“边缘距离”可在 `0–40dp` 调整抓手横向位置，拖动时轨道实时移动，松手后保存。低于系统手势安全参考线时会提示返回冲突风险，但不会阻止用户贴近物理边缘。
