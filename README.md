...
- 最近应用模式每次展开时刷新，手机最多显示 6 个、平板最多显示 10 个；未授予使用情况访问权限时使用侧栏启动历史作为回退。
- 固定应用模式按用户保存的顺序显示全部有效应用；在设置页点击“管理”或在侧栏点击“+”即可增删，超出可见行时可上下滚动。
- 最近应用模式不显示“+”；最近记录为空时显示历史图标，点击可返回设置与授权入口。
